...
import sqlite3 as sq3 # importa la librería que maneja la BBDD
...
#cur.executemany('INSERT INTO escuelas VALUES (?,?,?,?,?)', lista1) #notación paramétrica
#cur.executemany('INSERT INTO alumnos VALUES (?,?,?,?,?,?,?)', lista2)

query1 = '''SELECT alumnos.legajo, alumnos.nombre, alumnos.nota, 
alumnos.email, escuelas.nombre, escuelas.localidad, escuelas.provincia FROM alumnos
INNER JOIN escuelas ON alumnos.id_escuela = escuelas._id'''

for registro in cur.execute(query1):
    print(registro)

con.commit() # ejecuta los cambios en la BBDD
